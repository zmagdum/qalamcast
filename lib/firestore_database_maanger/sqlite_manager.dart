...
''');
  ...
  Future<List<Series>> getFavouriteSeries([String searchTerm]) async {
    ...
    var series = seriesMaps.map((e) => Series.initFromJson(e)).toList();
    return series;
  }
  ...
  Future<int> deleteEpisode(Episode episode) async {
    return await db.delete(EpisodeTable.tableName,
        where: '${EpisodeTable.id} = ?', whereArgs: [episode.id]);
  }
  ...
  print(episodeMaps);
    var episodes = episodeMaps.map((e) => Episode.initFromJson(e)).toList();
    return episodes;
  }
  ...
  Future<List<Episode>> getFavouriteEpisodes([String searchTerm]) async {
    ...
    var episode = episodeMaps.map((e) => Episode.initFromJson(e)).toList();
    return episode;
  }
